% To test the codes, we may need to change the dataDir and change the
% comment to run certain function
...
dataDir = fullfile('EECS442','hw1-finish','Extra part','data','prokudin-gorskii');

% Path to your output directory (change this to your output directory)
outDir = fullfile('EECS442','hw1-finish','Extra part', 'output', 'prokudin-gorskii');
...
for i = 1:length(imageNames),
    ...
    [colorIm, predShift] = alignChannels_extra1(channels, maxShift);
   %[colorIm, predShift] = alignChannels_extra2(channels, maxShift);
   %[colorIm, predShift] = alignChannels_extra3(channels, maxShift);
