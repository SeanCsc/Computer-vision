...
function mosim = demosaicTrans(im)
...
filter_r = [0,0,0;0,1,1;0,1,1];
filter_b = [1,1,0;1,1,0;0,0,0];
mask_r(im_g==0) = im_r(im_g==0);
mask_r(im_g~=0) = im_r(im_g~=0)./im_g(im_g~=0);
multi_r = conv2(mask_r,filter_r,'same');
mask_b(im_g==0) = im_b(im_g==0);
mask_b (im_g~=0) = im_b(im_g~=0)./im_g(im_g~=0);
multi_b = conv2(mask_b,filter_b,'same');
mosim(:,:,1) = multi_r.*im_g;
mosim(:,:,3) = multi_b.*im_g;
